...
nz = 450 #450 #2250 #int(2325*1.6)   # Number of grid points in z
nr = 15 #15 #75 #int(75*1.6)    # Number of grid points in r
...
# Time parameters
dt = np.float64(1e-6)  # Time step (s)
t_max = np.float64(0.002)  # Total simulation time (s)
...
with tqdm(total=n_steps, desc="\n", ncols=80) as pbar:
    ...
    for n in range(n_steps):
        ...
        u_r = U[:, :, 1] / (rho + 1e-10)
        u_z = U[:, :, 2] / (rho + 1e-10)
        duz_dz = np.gradient(u_z, dz, axis=0)  # Partial derivative of u_z in z
        dur_dr = np.gradient(u_r, dr, axis=1)  # Partial derivative of u_r in r
        div_v = duz_dz + dur_dr + u_r / (R + 1e-10)  # divergence
        tau_thetatheta = mu * (2 * u_r / (R + 1e-10) - (2/3) * div_v)  # theta-theta stress
        update[:, :, 1] -= dt * (Fr_visc[:, :, 1] - tau_thetatheta) / (R + 1e-10)  # (tau_rr - tau_{theta-theta}) / r
        update[:, :, 2] -= dt * Fr_visc[:, :, 2] / (R + 1e-10)  # tau_rz / r
        update[:, :, 3] -= dt * Fr_visc[:, :, 3] / (R + 1e-10)  # (k dT/dr + tau_rr u_r + tau_rz u_z) / r
        ...
        U[reset_vals, 2] = U[reset_vals, 0] * u_wind # Set wind
